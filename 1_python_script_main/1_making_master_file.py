...
def resolve_input(raw_dir: Path, preferred_name: str, legacy_names: list[str]) -> Path:
    candidates = [preferred_name] + legacy_names
    for name in candidates:
        p = raw_dir / name
        if p.exists():
            return p
    return raw_dir / preferred_name
...
ICGT_FILE = resolve_input(
    RAW_DIR,
    "icgt_genotype_phenotype_master.xlsx",
    ["Bekeley-structure analysis.xlsx"],
)
PR_SUPP_FILE = resolve_input(
    RAW_DIR,
    "pr_tars_validation_traits.xlsx",
    ["Supplementary Data 1_R1 ver.xlsx"],
)
TEMP_FILE = resolve_input(
    RAW_DIR,
    "pr_tars_climate_summary.csv",
    ["Temperature data-Brain's PR work.csv"],
)

required = [ICGT_FILE, PR_SUPP_FILE]
...
if TEMP_FILE.exists():
    ...
    print("[INFO] Optional climate summary file not found; skipping climate output.")
