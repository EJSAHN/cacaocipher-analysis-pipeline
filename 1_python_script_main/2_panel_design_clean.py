"""
Script 2 (public): Minimal SNP barcode design, QC analyses, and supplementary workbook regeneration.
...
Expected inputs (relative to BASE_DIR):
  ...
  data_raw/pr_tars_validation_traits.xlsx  (optional fallback for PR validation sheet)

Key outputs:
  master/barcode_panel_32_stats.csv
  master/panel_tradeoff_distance.csv
  master/panel_tradeoff_error.csv
  master/panel_pure_vs_traitaware.csv
...
RUN_BIT_BUDGET = False  # Optional: set to True to compute the MI/entropy bit-budget summary.

OUTPUT_FILE = BASE_DIR / "Supplementary Data 1.xlsx"
PR_RAW_FILE = (RAW_DIR / "pr_tars_validation_traits.xlsx") if (RAW_DIR / "pr_tars_validation_traits.xlsx").exists() else (RAW_DIR / "Supplementary Data 1_R1 ver.xlsx")
...
# ------------------------------
# Trait correlation (single trait) and trait-aware panel
# ------------------------------
...
def select_trait_aware_panel(
    ...
    return selected, sel_df
trait_markers_32, trait_panel_stats = select_trait_aware_panel(
    icgt_geno,
    snp_cols,
    trait_df,
    max_markers=32,
...
if 'trait_markers_32' in globals():
    ...
    trait32_summary["panel"] = "TraitAware_32"
...
# ------------------------------
# Multi-trait scan (trait-aware 32 panel per phenotype column)
# ------------------------------
numeric_cols = [
...
# ------------------------------
# Regenerate Supplementary Data 1 workbook
# ------------------------------
print("[INFO] Regenerating Supplementary Data 1 workbook...")
...
print(f"[INFO] Loading PR validation sheet from: {PR_RAW_FILE}")
try:
    try:
    ...
    print(f"[INFO] PR validation sheet loaded. Rows: {len(df_pr)}")
    
except Exception as e:
    print("[WARN] PR validation sheet could not be loaded. Check the raw file path.")
    print(f"[WARN] {e}")
...
print("\n" + "="*60)
print("[DONE] Supplementary workbook generated.")
print(f"[OPEN] Verify output: {OUTPUT_FILE}")
print("="*60)
