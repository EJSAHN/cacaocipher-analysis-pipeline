"""Build master tables from local raw inputs for the CacaoCipher analysis pipeline."""
...
def resolve_input(raw_dir: Path, preferred_name: str, *, contains_any: list[str], suffix: str) -> Path:
    ...
    raise FileNotFoundError(
        f"Missing required input in {raw_dir}. Expected `{preferred_name}` or another compatible {suffix} file."
    )


def resolve_optional_inputs(raw_dir: Path, *, contains_any: list[str], suffix: str) -> list[Path]:
    out: list[Path] = []
...
ICGT_FILE = resolve_input(
    RAW_DIR,
    "icgt_structure_analysis.xlsx",
    contains_any=["structureanalysis", "icgtstructure", "bekeleystructure", "structure"],
    suffix=".xlsx",
)
PR_SUPP_FILE = resolve_input(
    RAW_DIR,
    "pr_validation_traits.xlsx",
    contains_any=["prvalidation", "tarsgenotypes", "tars", "validation", "supplementarydata"],
    suffix=".xlsx",
)
TEMP_FILE = resolve_input(
    RAW_DIR,
    "pr_climate_daily.csv",
    contains_any=["prclimate", "temperature", "climate", "brainsprwork"],
    suffix=".csv",
)

try:
    GRIN_FILE = resolve_input(
        RAW_DIR,
        "grin_methods_global.xlsx",
        contains_any=["grin", "methodsgrin", "gringlobal"],
        suffix=".xlsx",
    )
except FileNotFoundError:
    GRIN_FILE = None

ALIGN_FILES = resolve_optional_inputs(
    RAW_DIR,
    contains_any=["align", "alignment", "correlation", "osorio", "studystructure"],
    suffix=".xlsx",
)
...
def load_pr_pheno(path: Path) -> pd.DataFrame:
    ...
    for sheet in xls.sheet_names:
        ...
        score = sum(any(key in c for c in cols_lower) for key in keys)
        sname = sheet.lower()
        for token in ["trait", "field", "tars", "phenotype", "2007", "2008", "2009", "2010", "2011"]:
            if token in sname:
                score += 1
        if score > best_score:
            best_score = score
    ...
    if target_sheet is None:
        raise ValueError("Could not find a compatible PR validation sheet.")
...
icgt_all, icgt_pheno, icgt_geno = load_icgt_table(ICGT_FILE)
...
if GRIN_FILE is not None and GRIN_FILE.exists():
    try:
    ...
    print("[INFO] Optional GRIN workbook not found.")
...
print(f"[SAVE] accession_overlap_auto.csv (overlap {len(overlap2)})")
...
print(f"PR accessions               : {pr['Accession_ID'].nunique()}")
