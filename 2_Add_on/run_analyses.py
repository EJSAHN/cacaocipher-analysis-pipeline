...
def run_benchmarks(
    ...
    *,
    ...
    if "TraitAware_32" in fixed_panels:
        panels.append(PanelSpec(name="Trait-enriched_heuristic", k=32, markers=fixed_panels["TraitAware_32"]))
    ...
    labels = []
    data = []
    grouped = list(pop.groupby(["panel", "k"]))
    positions = np.arange(1, len(grouped) + 1)
    for (panel, k), sub in grouped:
        labels.append(pretty_panel_label(f"{panel} (k={int(k)})"))
    ...
    if "TraitAware_32" in fixed_panels:
        trait_panels.append(PanelSpec(name="Trait-enriched_heuristic", k=32, markers=fixed_panels["TraitAware_32"]))
    if "PureID_96" in fixed_panels:
    ...
    labels = []
    data = []
    grouped = list(pred.groupby(["panel", "k"]))
    positions = np.arange(1, len(grouped) + 1)
    for (panel, k), sub in grouped:
        labels.append(pretty_panel_label(f"{panel} (k={int(k)})"))
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", type=str, default="2_Add_on/addon_outputs", help="Output directory relative to --project-root.")
    ap.add_argument("--master-dir", type=str, default="1_python_script_main/master", help="Directory containing the master CSV files.")
